...
class App extends React.Component {
  ...
  getmovieslist=()=>{
    ...
    fetch(`http://www.omdbapi.com/?s=${searchkeyword}&apikey=7da3fb5d`)
    .then(resultdata => resultdata.json())
    .then(resultdata => {
      if(this.isMounted){
        if (resultdata.Search && resultdata.Search.length) {
         let listofmovies = resultdata.Search.map(item => item.imdbID);
    ...
    return (
      <div className="app">
        ...
        <div className="search-box">
            ...
            <a className="search-icon" onClick={this.getmovieslist}><i className="fa fa-search"></i></a>
        </div>
